...
export const Card: FC<PropsCardType> = ({title, date, userName, status}) => {
    ...
    return (
        <div className='text-center bg-background-paper shadow border rounded-2xl px-5 pt-10 pb-2 relative'>
            {status &&
                <div className={`${s.absoluteVerified} bg-secondary-main text-secondary-contrastText`}>
                    verified
                </div>
            }
            <div className='grid justify-items-center'>
                ...
                <p className='mt-0 mb-7'><span className='text-text-primaryAlpha300'>By </span>{userName}</p>
                <Button href="#contained-buttons">
                    Start
                </Button>
                <p className='mb-8'><span className='text-text-primaryAlpha300'>Created: </span>{date}</p>
            </div>
        </div>)
